...
module sizeLabel(distance,
    ...
    rotate([0, rotation, 0])
        translate([0, 0, (over ? 1 : -1) * markerRadius + 0.01])
        color(color) {
...
function keyChildInfo(name, count, size) = [name, count, size];
...
// are vec3 denoting [N, E, S] how many sizeLabels are stacked on that
...
//       created (use an empty node, and set rightLabels=undef if you
...
// creates each projection view.  The taRightSide and taTopSide
...
module thirdAngle(size,
    ...
    assert($children >= 2 && $children <=4,
        ...
        translate([0,
                   0,
                   sizeLabelHeight()
                   * (frontLabels == undef ? 0 : frontLabels[2])]) {
        // Front view
        children([0:1]);

        // Right view
        frontRightLabelSpace = sizeLabelHeight()
            * (frontLabels == undef ? 0 : frontLabels[1]);
